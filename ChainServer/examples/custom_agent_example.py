"""
FlowForge Example: Custom Agent Registration

This example shows how users can register their own MCP servers or
custom agents with FlowForge and chain them into existing flows.

This demonstrates the EXTENSIBILITY of FlowForge.
"""
...
from flowforge.connectors.base import ConnectorConfig
...
forge = FlowForge(name="custom_demo")
...
class MyMCPServerAgent(MCPAgent):
    """
    Example of integrating an external MCP server as a FlowForge agent.

    This allows users to bring their own MCP servers into FlowForge chains.
    """
...
async def fetch_custom_data(ctx: ChainContext) -> dict[str, Any]:
    ...
    agent = forge.get_agent("my_custom_data_source")
...
async def enrich_with_mcp(ctx: ChainContext) -> dict[str, Any]:
    ...
    agent = forge.get_agent("my_mcp_server")
    if agent:
...
async def main():
    print("\n" + "=" * 60)
    print("FlowForge: Custom Agent Demo")
    print("=" * 60 + "\n")

    # Show what's registered
    print("Registered Agents:", forge.list_agents())
    print("Registered Steps:", forge.list_steps())
    print("Registered Chains:", forge.list_chains())
    print()
