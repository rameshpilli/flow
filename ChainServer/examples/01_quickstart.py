...
"""
================================================================================
01_QUICKSTART.PY - Your First AgentOrchestrator Chain
================================================================================

PURPOSE:
    This is the starting point for learning AgentOrchestrator. Start here!

WHAT YOU'LL LEARN:
    1. Creating an AgentOrchestrator instance
    2. Defining steps with @ao.step decorator
    3. Setting dependencies between steps (deps parameter)
    4. Defining chains with @ao.chain decorator
    5. Running chains with ao.launch()
    6. Using resumable chains with checkpointing

NEXT STEPS:
    After this, go to: 02_custom_agents.py

RUN THIS EXAMPLE:
    python examples/01_quickstart.py

    OR via CLI:
    ao run quickstart_chain
"""
...
# Create AgentOrchestrator instance (isolated to prevent registry bleed)
ao = AgentOrchestrator(name="quickstart", isolated=True)
...
class QuickstartChain:
    """Simple chain demonstrating AgentOrchestrator basics."""
...
async def main():
    ...
    print("\n" + "═" * 60)
    print("  AgentOrchestrator Quickstart Example")
    print("═" * 60 + "\n")

    # Validate the chain
    print("📋 Validating chain...")
    ao.check("QuickstartChain")

    # Run the chain
    print("\n🚀 Running chain...\n")
    result = await ao.launch("QuickstartChain", {"company": "Apple"})
...
async def demo_resumable():
    ...
    result = await ao.launch_resumable("QuickstartChain", {"company": "Microsoft"})
    ...
    # List runs
    print("\n📋 Listing recent runs...")
    runs = await ao.list_runs(limit=5)
    for run in runs:
