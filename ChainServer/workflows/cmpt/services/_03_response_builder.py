...
from workflows.cmpt.services._02_content_prioritization import ToolName
...
class ResponseBuilderService:
    ...
    async def _execute_agents(self, subqueries: list[Subquery]) -> list[AgentResult]:
        ...
        async def fetch_one(sq: Subquery) -> AgentResult:
            ...
            if not agent:
                logger.debug(f"[{sq.agent}] No agent found, using mock data")
                return self._get_mock_result(sq)

            try:
                logger.info(f"[{sq.agent}] Fetching: {sq.query} with params: {sq.params}")
                ...
                items = ao_result.data if isinstance(ao_result.data, list) else (data.get("items", []) if isinstance(data, dict) else [])
                item_count = len(items) if isinstance(items, list) else 0

                logger.info(f"[{sq.agent}] Response: success={ao_result.success}, items={item_count}, duration={ao_result.duration_ms:.0f}ms")
                if ao_result.error:
                    logger.warning(f"[{sq.agent}] Error: {ao_result.error}")
                ...
                logger.error(f"[{sq.agent}] Exception: {e}")
