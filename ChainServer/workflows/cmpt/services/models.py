...
Pydantic models for all CMPT chain services.
"""
...
class ResponseBuilderOutput(BaseModel):
    ...
    # Aggregated data
    all_items: list[dict[str, Any]] = Field(default_factory=list, description="All retrieved items combined")
    total_items: int = Field(0, ge=0, description="Total items retrieved")

    # Generated content
    financial_metrics: dict[str, Any] | None = Field(None, description="Extracted financial metrics")
    strategic_analysis: dict[str, Any] | None = Field(None, description="Generated strategic analysis")
    prepared_content: str | None = Field(None, description="Final prepared meeting content")

    # Parsed data from agents
    parsed_data_agent_chunks: dict[str, str] | None = Field(None, description="Parsed content from each agent")

    # Validation
    validation_results: dict[str, Any] | None = Field(None, description="Validation results for financial metrics")
    ...
    # Timing
    timing_ms: dict[str, float] | None = Field(None, description="Timing for each agent and LLM call")
...
class CitationDict(BaseModel):
    ...
    source_agent: list[str] = Field(description="List of data agents used. Valid values: 'SEC_agent', 'earnings_agent', 'news_agent'")
    source_content: list[str] = Field(description="List of verbatim quotes from source chunks. DO NOT paraphrase.")
    reasoning: str = Field(description="Explanation of how you derived this metric.")
...
class StrategicAnalysisResponse(BaseModel):
    ...
    strength: list[str] = Field(default_factory=list, description="4-6 key competitive strengths from SWOT analysis")
    weakness: list[str] = Field(default_factory=list, description="4-6 key vulnerabilities from SWOT analysis")
    opportunity: list[str] = Field(default_factory=list, description="4-6 growth opportunities from SWOT analysis")
    threat: list[str] = Field(default_factory=list, description="4-6 external threats from SWOT analysis")
    investment_thesis: list[dict[str, list[str]]] = Field(default_factory=list, description="Investment thesis as list of dicts with subheading -> bullet points")
    key_risk_highlights: list[str] = Field(default_factory=list, description="5-7 critical risks that could impact investment thesis")
    strategic_opportunities: list[dict[str, list[str]]] = Field(default_factory=list, description="Strategic opportunities for company and RBC engagement")
    recent_developments: list[dict[str, Any]] = Field(default_factory=list, description="Recent firm-level developments with category, header, date, description, source_url")
    sources: list[str] = Field(default_factory=list, description="List of all sources cited in the analysis")
    news_summary: list[dict[str, list[str]]] = Field(default_factory=list, description="Summary of recent news organized by theme")
    key_highlights: list[dict[str, list[str]]] = Field(default_factory=list, description="Key highlights from latest quarterly earnings")