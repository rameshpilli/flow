...
class Config:
    ...
    # MCP Agent URLs (individual env vars for easier access)
    SEC_MCP_URL = os.getenv("SEC_MCP_URL")
    EARNINGS_MCP_URL = os.getenv("EARNINGS_MCP_URL")
    NEWS_MCP_URL = os.getenv("NEWS_MCP_URL")
    ...
    @classmethod
    def get_mcp_config(cls) -> dict[str, str | None]:
        """Get MCP agent URLs for CMPT chain."""
        return {
            "sec_url": cls.SEC_MCP_URL,
            "earnings_url": cls.EARNINGS_MCP_URL,
            "news_url": cls.NEWS_MCP_URL,
        }
    
    @classmethod
    def has_mcp_agents(cls) -> bool:
        """Check if any MCP agent URLs are configured (via individual env vars or AGENT_CONFIG JSON)."""
        # Check individual env vars
        if any([cls.SEC_MCP_URL, cls.EARNINGS_MCP_URL, cls.NEWS_MCP_URL]):
            return True
        
        # Check AGENT_CONFIG JSON
        agents = cls.get_agents()
        return len(agents) > 0 and any(agent.get("mcp_url") for agent in agents)
