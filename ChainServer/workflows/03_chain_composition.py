"""
================================================================================
03_CHAIN_COMPOSITION.PY - Building Complex Pipelines from Sub-Chains
================================================================================

PURPOSE:
    Learn how to build modular, reusable pipelines by composing
    smaller chains into larger workflows.

WHAT YOU'LL LEARN:
    1. Creating sub-chains (reusable pipeline components)
    2. Composing chains within other chains
    3. Context scoping (isolating context between sub-chains)
    4. Parent chains that orchestrate sub-chains
    5. Parallel execution of independent sub-chains

PREREQUISITES:
    - Complete 01_quickstart.py and 02_custom_agents.py first

NEXT STEPS:
    After this, go to: 04_meeting_prep_chain.py

RUN THIS EXAMPLE:
    python examples/03_chain_composition.py
"""
...
ao = AgentOrchestrator(name="composition_example", version="1.0.0")
...
@ao.step(name="calculate_metrics", deps=["join_data"])
...
@ao.chain(name="data_transformation")
...
@ao.chain(name="report_generation")
...
@ao.step(name="finalize", deps=["__subchain__report_generation"])
...
@ao.chain(name="full_analytics_pipeline")
...
async def main():
    print("\n" + "=" * 60)
    print("     AgentOrchestrator Chain Composition Example")
    print("=" * 60 + "\n")
    ...
    result = await ao.launch("full_analytics_pipeline")
