...
│   ├── context_builder.py        # Stage 1 service implementation
│   ├── content_prioritization.py # Stage 2 service implementation
│   ├── response_builder.py       # Stage 3 service implementation
│   ├── grid_config.py            # Temporal priority rules
│   ├── static_subquery_engine.py # Query generation for agents
│   ├── llm_prompts.py            # Financial metrics & strategic prompts
│   └── validation_utils.py       # Metrics validation
│
...
CMPT-specific agents that extend AgentOrchestrator's `BaseAgent`:
...
class SECFilingAgent(MCPAgent):
    async def fetch(self, query: str, **kwargs) -> AgentResult:
        # Fetch SEC filings via MCP
        ...

class EarningsAgent(MCPAgent):
    async def fetch(self, query: str, **kwargs) -> AgentResult:
        # Fetch earnings transcripts via MCP
        ...

class NewsAgent(MCPAgent):
    async def fetch(self, query: str, **kwargs) -> AgentResult:
        # Fetch news articles via MCP
        ...
```
...
```python
from agentorchestrator import AgentOrchestrator
from examples.cmpt.chain import register_cmpt_chain
...
```python
from examples.cmpt.chain import run_cmpt_chain
