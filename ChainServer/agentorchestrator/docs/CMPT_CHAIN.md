# AgentOrchestrator CMPT Chain - Complete Flow Guide
...
│   │  File: agentorchestrator/services/context_builder.py                        │  │
│   │  ─────────────────────────────────────────────────────────────────  │  │
│   │  Extractors:                                                         │  │
...
│   │  File: agentorchestrator/services/content_prioritization.py                 │  │
│   │  ─────────────────────────────────────────────────────────────────  │  │
│   │  Components:                                                         │  │
...
│   │  File: agentorchestrator/services/response_builder.py                       │  │
│   │  ─────────────────────────────────────────────────────────────────  │  │
│   │  Components:                                                         │  │
...
```python
# File: agentorchestrator/chains/cmpt.py
...
**File:** `agentorchestrator/services/context_builder.py`
**Models:** `agentorchestrator/services/models.py`
...
### New Approach (AgentOrchestrator)
...
**File:** `agentorchestrator/middleware/summarizer.py`
...
# With OpenAI
from agentorchestrator.middleware.summarizer import create_openai_summarizer
forge.use_middleware(SummarizerMiddleware(
    summarizer=create_openai_summarizer(model="gpt-4"),
    max_tokens=4000,
))
...
from agentorchestrator.middleware.summarizer import create_anthropic_summarizer
forge.use_middleware(SummarizerMiddleware(
...
AgentOrchestrator includes several enterprise-grade features for production deployments.

### CLI Tool

AgentOrchestrator provides a command-line interface for common operations:

```bash
# Run a chain
agentorchestrator run cmpt_chain --data '{"company": "Apple"}'
...
# Visualize DAG
agentorchestrator graph cmpt_chain
agentorchestrator graph cmpt_chain --format mermaid
...
agentorchestrator debug cmpt_chain --data '{"company": "Apple"}'
agentorchestrator debug cmpt_chain --snapshot-dir ./debug_output

# Health check (returns exit code 1 if unhealthy)
agentorchestrator health
agentorchestrator health --json
...
```bash
agentorchestrator debug my_pipeline --data '{"company": "Apple"}'
...
```python
from agentorchestrator import CircuitBreaker, CircuitBreakerConfig, get_circuit_breaker
...
```python
from agentorchestrator import LLMGatewayClient, create_managed_client
...
forge = AgentOrchestrator(name="cmpt")
...
```python
from agentorchestrator import configure_logging, get_logger, ChainLogger
...
AgentOrchestrator includes built-in OpenTelemetry tracing that's automatically integrated at the DAG executor layer:

```python
from agentorchestrator import AgentOrchestrator, configure_tracing, ChainTracer, trace_span
...
AgentOrchestrator implements true fail-fast behavior that immediately cancels pending tasks:
...
```python
from agentorchestrator.core.context import StepResult, ExecutionSummary
...
Configure AgentOrchestrator with environment variables and automatic secret masking:

```python
from agentorchestrator.utils import get_config, AgentOrchestratorConfig, SecretString
...
aws_backend = AWSSecretsManagerBackend(
    secret_name="agentorchestrator/prod",
    region_name="us-east-1",
    cache_ttl_seconds=300,  # Cache for 5 minutes
)
config = AgentOrchestratorConfig.from_env(secret_backend=aws_backend)
...
config = AgentOrchestratorConfig.from_env(secret_backend=vault_backend)
```
...
```python
from agentorchestrator.utils import get_health, get_version
...
# Get version info
version = get_version()   # {"name": "agentorchestrator", "version": "1.0.0", ...}
```
...
agentorchestrator health || exit 1

# Dockerfile example
HEALTHCHECK --interval=30s --timeout=3s \
  CMD agentorchestrator health || exit 1
```
...
AgentOrchestrator uses isolated registries by default to prevent state bleed:

```python
from agentorchestrator import AgentOrchestrator

# Isolated by default (safe for testing)
forge = AgentOrchestrator(name="my_app")  # isolated=True is default

# Explicit shared registries (for global singleton)
forge = AgentOrchestrator(name="my_app", isolated=False)
...
```python
from agentorchestrator.core.registry import get_step_registry, get_chain_registry
...
The decorators in `agentorchestrator/core/decorators.py` delegate to the global AgentOrchestrator instance:
...
This provides a clean, simple API for quick scripts while still allowing full control via the `AgentOrchestrator` class when needed.
...
| File | Purpose |
|------|---------|
| `agentorchestrator/__init__.py` | Package exports and usage examples |
| `agentorchestrator/core/forge.py` | Main AgentOrchestrator class, decorators, execution, cleanup timeout |
| `agentorchestrator/core/dag.py` | DAG builder and executor (supports explicit parallel_groups) |
| `agentorchestrator/core/context.py` | ChainContext, StepResult, ExecutionSummary |
| `agentorchestrator/core/registry.py` | Agent, Step, Chain registries (isolated by default, strict mode) |
| `agentorchestrator/core/decorators.py` | @agent, @step, @chain decorators (delegate to global forge) |
| `agentorchestrator/core/resources.py` | Resource management and dependency injection |
| `agentorchestrator/core/visualize.py` | DAG visualization (ASCII and Mermaid) |
| `agentorchestrator/cli.py` | Command-line interface (run, check, list, graph, new, dev) |

### CMPT Chain

| File | Purpose |
|------|---------|
| `agentorchestrator/chains/__init__.py` | Chain exports |
| `agentorchestrator/chains/cmpt.py` | CMPT chain definition and CMPTChain class |

### Services (Business Logic)

| File | Purpose | Stage |
|------|---------|-------|
| `agentorchestrator/services/__init__.py` | Service exports |  |
| `agentorchestrator/services/models.py` | All Pydantic models | All |
| `agentorchestrator/services/context_builder.py` | Context extraction | 1 |
| `agentorchestrator/services/content_prioritization.py` | Source prioritization | 2 |
| `agentorchestrator/services/response_builder.py` | Agent execution & response | 3 |

### Middleware

| File | Purpose |
|------|---------|
| `agentorchestrator/middleware/__init__.py` | Middleware exports |
| `agentorchestrator/middleware/base.py` | Base Middleware class |
| `agentorchestrator/middleware/summarizer.py` | Token/content summarization (stuff/map_reduce/refine strategies) |
| `agentorchestrator/middleware/cache.py` | Response caching |
| `agentorchestrator/middleware/logger.py` | Execution logging |
| `agentorchestrator/middleware/token_manager.py` | Token usage tracking |
...
| File | Purpose |
|------|---------|
| `agentorchestrator/utils/__init__.py` | Utility exports |
| `agentorchestrator/utils/circuit_breaker.py` | Circuit breaker pattern implementation |
| `agentorchestrator/utils/logging.py` | Structured logging (structlog integration) |
| `agentorchestrator/utils/tracing.py` | Distributed tracing (OpenTelemetry) |
| `agentorchestrator/utils/timing.py` | Timing decorators (@timed, @async_timed) |
| `agentorchestrator/utils/retry.py` | Retry with exponential backoff |
| `agentorchestrator/utils/config.py` | Typed configuration with secret masking |

**Key exports from `config.py`:**
- `AgentOrchestratorConfig` - Typed configuration dataclass
...
| File | Purpose |
|------|---------|
| `agentorchestrator/test_cmpt_chain.py` | End-to-end test suite |
...
agentorchestrator run cmpt_chain --data '{"company": "Apple Inc"}'
...
# With custom port
agentorchestrator dev --watch --port 8000
```
...
This guide walks you through building a Client Meeting Prep (CMP) style flow using AgentOrchestrator. By the end, you'll have a working chain that:
...
- Python 3.11+
- AgentOrchestrator installed (`pip install agentorchestrator`)
...
```bash
# Create new project
agentorchestrator new project meeting_prep
...
from agentorchestrator.plugins.capability import CapabilitySchema, AgentCapability, CapabilityParameter


class CompanyAgent(BaseAgent):
    """Agent for fetching company information."""

    _agentorchestrator_name = "company_agent"
...
from agentorchestrator.plugins.http_adapter import HTTPAdapterAgent, HTTPAdapterConfig
...
def create_news_agent():
    ...
    agent._agentorchestrator_name = "news_agent"
...
from agentorchestrator import AgentOrchestrator, ChainContext, get_forge
...
```bash
# Validate the chain
agentorchestrator validate meeting_prep_chain

# Run with sample data
agentorchestrator run meeting_prep_chain --data '{"request": {"company_name": "Apple Inc"}}'

# Visualize the DAG
agentorchestrator graph meeting_prep_chain --format mermaid
```
