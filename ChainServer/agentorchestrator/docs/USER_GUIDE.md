# AgentOrchestrator User Guide

A comprehensive guide to building data pipelines and AI chains with AgentOrchestrator.
...
AgentOrchestrator is a DAG-based chain orchestration framework inspired by Dagster patterns. It provides:
...
# Install AgentOrchestrator in development mode
pip install -e .
```
...
AgentOrchestrator includes a command-line interface for common operations.

### Running Chains

```bash
# Run a chain with initial data
agentorchestrator run my_pipeline --data '{"company": "Apple"}'

# Run with a specific module
agentorchestrator run my_pipeline --module myapp.chains

# Run with verbose output
agentorchestrator run my_pipeline --verbose
```
...
agentorchestrator debug my_pipeline --snapshot-dir ./debug_output
...
agentorchestrator check --module myapp.chains
```
...
agentorchestrator list --verbose
```
...
agentorchestrator graph my_pipeline --format mermaid

# Output to file
agentorchestrator graph my_pipeline --format mermaid --output dag.md
```
...
```bash
# Create a new agent
agentorchestrator new agent MyCustomAgent

# Create a new chain
agentorchestrator new chain MyPipeline

# Specify output directory
agentorchestrator new agent MyAgent --output ./myapp/agents/
```
...
agentorchestrator dev --watch --path ./myapp/
```
...
agentorchestrator health --detailed  # Include external dependencies
...
agentorchestrator version --json

# Show configuration (secrets masked)
agentorchestrator config
agentorchestrator config --json
```
...
The `AgentOrchestrator` class is your entry point. It manages:
- Agent registration
- Step registration
- Chain registration
- Middleware
...
# Note: By default, AgentOrchestrator uses isolated registries to prevent
...
1. **Automatic Detection**: When you include a chain name in the `steps` list, AgentOrchestrator automatically detects it and wraps it as a subchain step
...
AgentOrchestrator provides base classes for common patterns:
...
When processing financial data, generic summarization loses important details. AgentOrchestrator provides **domain-aware prompts** that know what to extract.
...
```python
from agentorchestrator import SummarizerMiddleware, LangChainSummarizer
...
```python
from agentorchestrator import LangChainSummarizer
...
When processing financial data (SEC filings, news articles, earnings transcripts), individual step outputs can be massive (1-5MB+). AgentOrchestrator provides a **Redis Context Store** to keep chain context lightweight while **NEVER losing data**.
...
```python
from agentorchestrator.middleware import TokenManagerMiddleware, LangChainSummarizer
from agentorchestrator.core import RedisContextStore
...
```python
from agentorchestrator.middleware import cap_items_with_metadata, cap_per_source
...
AgentOrchestrator supports Pydantic model validation for fail-fast type checking.
...
AgentOrchestrator supports checkpointing and resuming chains for fault tolerance.

### Setting Up Run Store

```python
from agentorchestrator.core import FileRunStore, InMemoryRunStore, ResumableChainRunner
...
AgentOrchestrator includes enterprise-grade features for production deployments.
...
```python
from agentorchestrator import retry, async_retry, RetryPolicy
...
```python
from agentorchestrator import configure_logging, get_logger, LogContext, ChainLogger
...
```python
from agentorchestrator import AgentOrchestrator, configure_tracing, get_tracer, trace_span, ChainTracer
...
# agentorchestrator debug my_chain --company "Apple" --snapshot-dir ./debug_output
```
...
AgentOrchestrator implements true fail-fast behavior - when one task fails, all pending tasks are **immediately cancelled**:
...
# NEW behavior (AgentOrchestrator):
...
AgentOrchestrator provides a typed configuration system with automatic validation and secret masking.

### Loading Configuration

```python
from agentorchestrator.utils import AgentOrchestratorConfig, get_config, set_config
...
config = AgentOrchestratorConfig.from_env(secret_backend=env_backend)
```
...
```python
from agentorchestrator.utils import SecretString
...
```python
from agentorchestrator.utils import AgentOrchestratorConfig, ConfigError
...
AgentOrchestrator provides built-in health checks for operational visibility.

### Health Status API

```python
from agentorchestrator.utils import get_health, get_version, HealthStatus
...
```python
from agentorchestrator.utils import get_health, HealthStatus
...
AgentOrchestrator includes a pre-built Client Meeting Prep (CMPT) chain.
...
```python
"""
AgentOrchestrator Example: Capital IQ Integration
...
class AgentOrchestrator:
    ...
    # Context managers
    @classmethod
    def temp_registries(cls, name: str) -> AgentOrchestrator  # For isolated testing
...
AgentOrchestrator includes a comprehensive test suite and utilities for testing your own chains.
...
```python
from agentorchestrator.tests.conftest import (
    MockAgent,
    FailingAgent,
    TrackingMiddleware,
    create_simple_step,
...
*AgentOrchestrator - Building AI chains, one step at a time.*
