# FlowForge Troubleshooting Guide

Common issues and solutions for FlowForge applications.
...
flowforge debug my_chain --company "Apple" --snapshot-dir ./debug
```
...
3. Reset circuit manually:
   ```python
   from flowforge.utils.circuit_breaker import reset_all_circuit_breakers
...
3. Verify discovery:
   ```python
   from flowforge.plugins import discover_plugins
...
3. Use in-memory store for development:
   ```python
   from flowforge.core.context_store import InMemoryContextStore
...
1. Check run exists:
   ...
   flowforge run-info run_xxx
   ```
2. Verify checkpoint directory:
   ```python
   forge = FlowForge(checkpoint_dir="./checkpoints")
   ```
3. Check run status:
   ```python
...
1. Use migrations:
   ```python
   from flowforge.core.versioning import MigrationManager
...
2. Validate before migration:
   ```bash
   flowforge validate my_chain --data '{"old_format": "data"}'
   ```
...
1. Enable debug mode:
   ```bash
   flowforge debug my_chain --snapshot-dir ./debug
   ```
2. Check step timings in snapshots
3. Enable tracing:
   ```python
   from flowforge.utils.tracing import configure_tracing
...
1. Add caching:
   ```python
   from flowforge.middleware import CacheMiddleware
   forge.use_middleware(CacheMiddleware(ttl_seconds=300))
   ```
2. Increase parallelism:
   ```python
   forge = FlowForge(max_parallel=20)  # Default is 10
   ```
...
1. Check component health:
   ```bash
   flowforge health --detailed --verbose
   ```
...
2. **Run diagnostics:**
   ```bash
   flowforge health --detailed --json > health.json
   flowforge validate my_chain --json > validation.json
   ```

3. **Report issues:** https://github.com/flowforge/flowforge/issues
   - Include error message
   - Include FlowForge version: `flowforge version --json`
